...
# The output shape is [n_channels, n_timepoints]
...
# The data is already in (channels, time) format, so we use the default 'channels_first'
naive_results = nmi.run(
    x_data=x_raw,
    y_data=y_raw,
    mode='estimate',
    processor_type='continuous',
    processor_params={'window_size': 1, 'data_format': 'channels_first'},
...
sweep_results = nmi.run(
    ...
    mode='sweep',
    processor_type='continuous',
    processor_params={'data_format': 'channels_first'},
    base_params=base_params,
    sweep_grid=sweep_grid,
    n_workers=4  # Speed up the sweep with parallel workers
)

print(sweep_results.dataframe.head())
