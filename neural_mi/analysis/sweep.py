...
def _build_critic(critic_type, embedding_params, use_variational=False, custom_embedding_model=None):
...
def _run_training_task(args):
    ...
    # --- FIX: Extract the custom model from params and pass it to the builder ---
...
class ParameterSweep:
    ...
    This class now assumes it will only receive pre-processed, 3D data.
    """
    def __init__(self, x_data, y_data, base_params, critic_type='separable', 
        ...
        if x_data.ndim != 3:
            raise ValueError("ParameterSweep now only accepts pre-processed 3D data.")
        ...
        # --- SIMPLIFICATION: We can now reliably calculate input dimensions ---
        self.base_params['input_dim_x'] = x_data.shape[1] * x_data.shape[2]
        self.base_params['input_dim_y'] = y_data.shape[1] * y_data.shape[2]

    def run(self, sweep_grid, n_workers=None): # removed processor_type
        """
    ...
    def _prepare_tasks(self, sweep_grid): # removed processor_type
        ...
        for i_combo, params in enumerate(param_combinations):
            ...
            # --- SIMPLIFICATION: Data is already processed ---
