...
from neural_mi.utils import build_critic, get_device, _shift_data
...
def _process_task_data(x_data: Any, y_data: Any, params: Dict[str, Any]) -> Tuple[torch.Tensor, torch.Tensor]:
    ...
    # If data is already processed (3D tensor), just return it.
    if isinstance(x_data, torch.Tensor) and x_data.ndim == 3:
        return x_data, y_data

    proc_type_x = params.get('processor_type_x')
    proc_params_x = params.get('processor_params_x', {})
    proc_type_y = params.get('processor_type_y')
    proc_params_y = params.get('processor_params_y', {})

    if 'lag' in params and proc_type_x and proc_type_y:
        # For lag analysis, we assume y is the one being shifted.
        # Note: This logic might need adjustment if shifting spike times by float values.
        x_data, y_data = _shift_data(x_data, y_data, params['lag'], proc_type_y)

    handler = DataHandler(x_data, y_data, proc_type_x, proc_params_x, proc_type_y, proc_params_y)
