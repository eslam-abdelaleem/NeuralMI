...
- **`dimensionality.py`**: Implements the `mode='dimensionality'` analysis. *Note: This module trains a Hybrid Critic embedding independently `n_splits` times, extracts each run's rotated cross-covariance directions on held-out data, and cross-checks them for reproducibility (`_compute_stability_report`) and convergence — see THEORY.md §6 for why a single run's spectral metrics alone (PR, kept only as a secondary diagnostic) aren't treated as a dimensionality count.*
