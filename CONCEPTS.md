...
* **Hybrid Critic (`hybrid`):** Embeds $X$ and $Y$ independently to a specified `embedding_dim` (the bottleneck), concatenates those embeddings, and then passes them through a final multi-layer perceptron (MLP) decision head. **Used automatically by `mode='dimensionality'`**.



**Why do we need the Hybrid Critic?** The Hybrid Critic solves the "Critic Problem" that `mode='dimensionality'` depends on: if you force data through a small bottleneck using a `separable` critic, the rigid geometry of the dot product might fail to capture complex, non-linear dependencies, and a direction of shared structure would look absent when it's really just unreachable by a dot product. The `hybrid` critic guarantees that if the bottleneck is large enough to allow information to pass, the MLP decision head has the geometric flexibility to find it — which is what makes the cross-run stability check in `mode='dimensionality'` (see THEORY.md §6) trustworthy in the first place.
