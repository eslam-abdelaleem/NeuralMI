...
class Trainer:
    ...
    """
    def __init__(self, model, estimator_fn, optimizer, use_variational=False, beta=1.0):
        """
        Args:
            model (nn.Module): The critic model that produces a scores matrix.
            estimator_fn (callable): The MI bound function (e.g., infonce_lower_bound).
            optimizer (torch.optim.Optimizer): The optimizer for training.
            use_variational (bool): If True, uses the variational (DVSIB) loss.
            beta (float): The beta parameter for the variational objective.
        """
        self.model = model
    ...
    def train(self, x_data, y_data, n_epochs, batch_size,
        ...
        self.model.to(x_data.device)
        run_id = run_id if run_id is not None else str(uuid.uuid4())
        temp_model_path = f".{run_id}_best_model.pt"
        ...
        for epoch in range(n_epochs):
            ...
            print(f"Epoch {epoch+1}/{n_epochs} | Test MI: {test_mi:.4f}")

            # Early stopping check on the smoothed curve
        ...
        print(f"Best epoch identified (via smoothed curve): {best_epoch + 1} (Smoothed MI: {smoothed_history[best_epoch]:.4f})")
        
        self.model.load_state_dict(torch.load(temp_model_path, weights_only=True))
        self.model.eval()
        with torch.no_grad():
    ...
    def _evaluate_final_mi(self, x, y):
        """Helper to evaluate MI, correctly handling standard and variational cases."""
        if x.shape[0] < 2:
            return float('nan')
    ...
    def _smooth(self, mi_array, sigma, median_window):
        """Smooths the MI curve using median and gaussian filters."""
