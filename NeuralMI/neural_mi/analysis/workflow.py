...
class AnalysisWorkflow:
    ...
    def run(self, param_grid, gamma_range=range(1, 11), n_workers=None,
        ...
        final_results = self._post_process_and_correct(raw_results, delta_threshold, min_gamma_points, confidence_level, verbose)
        
        return final_results

    def _post_process_and_correct(self, raw_results, delta_threshold, min_gamma_points, confidence_level, verbose):
            ...
            corrected_results = []
            for params, group in df.groupby(param_cols):
                param_dict = dict(zip(param_cols, group.iloc[0][param_cols].values)) if param_cols else {}
                ...
                while len(gammas_to_fit) >= min_gamma_points:
                    ...
                    # The total weight for each gamma level should be equal.
                    # So, each point's weight is 1 / (number of points at its gamma level).
                ...
                if len(final_subset) < 2:
                    ...
                    # --- Apply Same Weighting Logic to Final Linear Fit ---
                    final_subset = group[group['gamma'].isin(gammas_to_fit)]
