...
_TRAIN = {
    'n_epochs': 1, 'learning_rate': 1e-4, 'batch_size': 8,
...
def test_dimensionality_defaults_shared_encoder_true():
    ...
    We verify this via nmi.run so that all base_params defaults are injected
    before run_dimensionality_analysis is called.  The test checks that the
    run succeeds (shared encoder should halve parameters and be valid here)
    and that no ValueError about shared_encoder is raised.
    """
    x, _ = nmi.generators.generate_nonlinear_from_latent(300, 3, 8, 1.0)
    results = nmi.run(
        x_data=x, y_data=None,
        mode='dimensionality',
        base_params={**_TRAIN},
        n_workers=1,
    )
...
def test_dimensionality_shared_encoder_can_be_overridden():
    ...
    results = nmi.run(
        x_data=x, y_data=None,
        mode='dimensionality',
        base_params={**_TRAIN, 'shared_encoder': False},
        n_workers=1,
    )
...
def test_run_shared_encoder_shortcut():
    """run(shared_encoder=True) should inject into base_params without error."""
